...
// Author: Lincoln D. Stein <lincoln.stein@gmail.com> AND Kar Ming Chu. Yup.
...
// Galaxy 
function send_data_to_galaxy(url_list_url) {
    var result ;
    var xmlhttp;
    var urls;
    var url_string;
    ...
    xmlhttp.onreadystatechange = function () {
         ...
         urls = result.match(/ftp.*fastq.gz/g);
			if (urls === null) {
				alert("No FASTQ files selected.");
				return;
			}
			// set info to be set via http request
			document.getElementById('URL').value = document.getElementById('URL').value + url_list_url;
			// submit if fastq files are selected
	 		document.getElementById('galaxyForm').submit();
	 	}
    }; 
	 // check output is valid
    xmlhttp.open("GET",url_list_url,true);
    xmlhttp.send();
}
...
function shopping_cart_check () {
	...
	// added for Galaxy 
	if ($('send_data_to_galaxy') == null)
	    buttons.insert(new Element('button',{id:"send_data_to_galaxy"}).update('Send Data to Galaxy'));

	$('clear_all').onclick = function() {clear_all()};
	//$('clear_all').enable();
	// added for Galaxy 
	$('send_data_to_galaxy').onclick = function() {send_data_to_galaxy(url_list_url)};
	$('send_data_to_galaxy').enable();
