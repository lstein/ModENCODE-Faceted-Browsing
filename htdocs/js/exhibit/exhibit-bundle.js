...
var processElmt=function(elmt){try{var component=Exhibit.UI.createFromDOM(elmt,uiContext);
...
var _processElmts=function(elmts,start){if(start<elmts.length){setTimeout(function(){processElmt(elmts[start]);
_processElmts(elmts,start+1);
},1);
}};
var processElmts=function(elmts){_processElmts(elmts,0);
};
...
Exhibit._Impl.prototype.processElmts=function(E,D){for(var C=0;
C<E.length;
C++){var B=E[C];
try{var A=Exhibit.UI.createFromDOM(B,uiContext);
if(A!=null){var G=B.id;
if(G==null||G.length==0){G="component"+Math.floor(Math.random()*1000000);
}this.setComponent(G,A);
}}catch(F){SimileAjax.Debug.exception(F.message);
}}};
...
Exhibit.ListFacet.createFromDOM=function(A,D,G){var C=Exhibit.getConfigurationFromDOM(A);
var G=Exhibit.UIContext.createFromDOM(A,G);
var B=new Exhibit.ListFacet(D!=null?D:A,G);
Exhibit.SettingsUtilities.collectSettingsFromDOM(A,Exhibit.ListFacet._settingSpecs,B._settings);
try{var E=Exhibit.getAttribute(A,"expression");
if(E!=null&&E.length>0){B._expression=Exhibit.ExpressionParser.parse(E);
}var N=Exhibit.getAttribute(A,"facetlabel");
var F=document.location.search.substr(1).split("&");
for(var H=0;
H<F.length;
H++){var M=F[H].split("=");
var K=unescape(M[0]);
var L=unescape(M[1]);
if(K==N){A.writeAttribute("ex:selection",L);
}}var O=Exhibit.getAttribute(A,"selection",";");
if(O!=null&&O.length>0){for(var H=0,P;
P=O[H];
H++){B._valueSet.add(P);
}}var I=Exhibit.getAttribute(A,"selectMissing");
if(I!=null&&I.length>0){B._selectMissing=(I=="true");
}}catch(J){SimileAjax.Debug.exception(J,"ListFacet: Error processing configuration of list facet");
}Exhibit.ListFacet._configure(B,C);
B._initializeUI();
G.getCollection().addFacet(B);
return B;
};
Exhibit.ListFacet._configure=function(F,E){Exhibit.SettingsUtilities.collectSettings(E,Exhibit.ListFacet._settingSpecs,F._settings);
if("expression" in E){F._expression=Exhibit.ExpressionParser.parse(E.expression);
}if("selection" in E){var C=E.selection;
for(var B=0;
B<C.length;
B++){F._valueSet.add(C[B]);
}}if("selectMissing" in E){F._selectMissing=E.selectMissing;
}if(!("facetLabel" in F._settings)){F._settings.facetLabel="";
}if("fixedOrder" in F._settings){var A=F._settings.fixedOrder.split(";");
var D={};
for(var B=0;
B<A.length;
B++){D[A[B].trim()]=B;
}F._orderMap=D;
}if("colorCoder" in F._settings){F._colorCoder=F._uiContext.getExhibit().getComponent(F._settings.colorCoder);
}if(F._settings.collapsed){F._settings.collapsible=true;
}F._cache=new Exhibit.FacetUtilities.Cache(F._uiContext.getDatabase(),F._uiContext.getCollection(),F._expression);
};
...
}this._slider.updateHistogram(E,G);
}};
...
this._minSlider={};
this._maxSlider={};
this._dom=SimileAjax.DOM.createDOMFromString(C,'<div class="exhibit-slider-bar" id="bar"><div class="exhibit-slider-handle" id="minHandle"></div><div class="exhibit-slider-handle" id="maxHandle"></div>'+(this._facet._settings.histogram?'<canvas class="exhibit-slider-histogram" id="histogram"></canvas>':"")+'</div><div class="exhibit-slider-display"><span id="minDisplay"></span> - <span id="maxDisplay"></span></div>');
...
document.attachEvent("onmouseup",E(F,onMove));
...
Exhibit.SliderFacet.slider.prototype.updateHistogram=function(D,B){var A=this._dom.bar.offsetWidth/B;
...
Exhibit.Lens._performConstructFromLensTemplateJob=function(E){Exhibit.Lens._constructFromLensTemplateNode({"value":E.itemID},{"value":"item"},E.template.template,E.div);
var D=E.div.lastChild;
var B=D.tagName;
if(B=="span"){D.style.display="inline";
}else{D.style.display="block";
}D.setAttribute("ex:itemID",E.itemID);
if(!Exhibit.params.safe){var A=Exhibit.getAttribute(D,"onshow");
if(A!=null&&A.length>0){try{var C=new Function(A);
C.call(D);
}catch(F){SimileAjax.Debug.log(F.message);
}}}return D;
};
...
this._offset=0;
var A=this;
this._listener={onItemsChanged:function(){A._offset=0;
A._reconstruct();
}};
B.getCollection().addListener(this._listener);
};
Exhibit.TabularView._settingSpecs={"sortAscending":{type:"boolean",defaultValue:true},"sortColumn":{type:"int",defaultValue:0},"showSummary":{type:"boolean",defaultValue:true},"showToolbox":{type:"boolean",defaultValue:true},"border":{type:"int",defaultValue:1},"cellPadding":{type:"int",defaultValue:5},"cellSpacing":{type:"int",defaultValue:3},"maxRows":{type:"int",defaultValue:0}};
...
if(s!=null&&s.length>0){f=eval(s);
if(typeof f=="function"){view._settings.tableMunger=f;
...
Exhibit.TabularView.prototype._reconstruct=function(){var L=this;
var K=this._uiContext.getCollection();
var I=this._uiContext.getDatabase();
var E=this._dom.bodyDiv;
E.innerHTML="";
var M=[];
var A=K.countAllItems();
if(A>0){var D=K.getRestrictedItems();
D.visit(function(T){M.push({id:T,sortKey:""});
});
}var Q;
if($("navigate")!=null){$("navigate").innerHTML="";
Q=$("navigate");
}else{Q=new Element("span",{id:"navigate"});
E.insert({before:Q});
}if(M.length>0){var G=this._columns[this._settings.sortColumn];
M.sort(this._createSortFunction(M,G.expression,this._settings.sortAscending));
var R=document.createElement("table");
R.className="exhibit-tabularView-body";
if(this._settings.tableStyler!=null){this._settings.tableStyler(R,I);
}else{R.cellSpacing=this._settings.cellSpacing;
R.cellPadding=this._settings.cellPadding;
R.border=this._settings.border;
}var B=R.insertRow(0);
var F=function(T){var U=L._columns[T];
if(U.label==null){U.label=L._getColumnLabel(U.expression);
}var V=document.createElement("th");
Exhibit.TabularView.createColumnHeader(exhibit,V,U.label,T==L._settings.sortColumn,L._settings.sortAscending,function(X,W,Y){L._doSort(T);
...
for(var P=0;
P<this._columns.length;
P++){F(P);
}var H;
if(this._rowTemplate!=null){H=function(T){var U=M[T];
var V=Exhibit.Lens.constructFromLensTemplate(U.id,L._rowTemplate,R,L._uiContext);
if(L._settings.rowStyler!=null){L._settings.rowStyler(U.id,I,V,T-L._offset);
}};
}else{H=function(U){var W=M[U];
var X=R.insertRow(R.rows.length);
for(var a=0;
a<L._columns.length;
a++){var V=L._columns[a];
var Z=X.insertCell(a);
var T=V.expression.evaluate({"value":W.id},{"value":"item"},"value",I);
var Y=V.format=="list"?T.valueType:V.format;
V.uiContext.formatList(T.values,T.size,Y,function(b){Z.appendChild(b);
});
if(V.styler!=null){V.styler(W.id,I,Z);
}}if(L._settings.rowStyler!=null){L._settings.rowStyler(W.id,I,X,U);
}};
}var O=+this._settings.maxRows;
var S=(O>0&&O+L._offset<M.length)?O:M.length-L._offset;
for(var P=0;
P<S;
P++){H(L._offset+P);
}E.appendChild(R);
if(L._offset>0){Q.insert('<div id="goback" class="exhibit-link" style="float:left">&lt;&lt; Previous '+O+"</div> ");
}if(O>0&&L._offset<M.length-O-1){Q.insert('<div id="goforward" class="exhibit-link" style="float:right">Next '+O+"&gt;&gt;</div>");
}var N=new Element("select",{name:"max-values"});
var C=[10,20,50,100,500,0];
C.each(function(T){var U=new Element("option",{value:T}).update(T==0?"All":T);
if(T==O){U.selected="selected";
}N.insert(U);
N.onchange=function(V){L._settings.maxRows=V.element().getValue();
L._offset=0;
L._reconstruct();
};
});
var J=L._offset+S;
Q.insert('<div class="exhibit-centered" id="tab-items-showing-label"><b>Showing items '+(L._offset+1)+"-"+J+",</b></div>");
$("tab-items-showing-label").insert("&nbsp;");
$("tab-items-showing-label").insert("Max items: ");
$("tab-items-showing-label").insert(N);
if($("goback")!=null){$("goback").onclick=function(){L._offset-=O;
L._reconstruct();
};
}if($("goforward")!=null){$("goforward").onclick=function(){L._offset+=O;
L._reconstruct();
};
}if(this._settings.tableMunger!=null){this._settings.tableMunger(R,I);
}}};
...
SimileAjax.WindowManager.registerEvent(F.facetLabel,"click",function(){Exhibit.FacetUtilities.toggleCollapse(F);
});
